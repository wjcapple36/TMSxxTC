    ...
    int count)
{
	...
	if (pcondition != NULL) {
		for (int i = 0; i < count; i++) {
			...
			sqlite3_prepare(db, sql, -1, &pstmt, 0);
			// 以二进制形式填写
			if (SQLITE_OK != sqlite3_bind_blob(pstmt, 1, pcondition->dev_name, 64, NULL) ) {
				fprintf(stdin, "sqlite3_bind_blob");
			}
			if (SQLITE_OK != sqlite3_bind_blob(pstmt, 2, pcondition->cable_name, 64, NULL) ) {
				fprintf(stdin, "sqlite3_bind_blob");
			}
			if (SQLITE_OK != sqlite3_bind_blob(pstmt, 3, pcondition->start_name, 64, NULL) ) {
				fprintf(stdin, "sqlite3_bind_blob");
			}
			if (SQLITE_OK != sqlite3_bind_blob(pstmt, 4, pcondition->end_name, 64, NULL) ) {
				fprintf(stdin, "sqlite3_bind_blob");
			}
