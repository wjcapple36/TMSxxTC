...
void CMD5::MD5Final(MD5_CTX *context)
{
...
char * CMD5::MD5(unsigned char *src, unsigned int inlen, char *pszHex)
{
	// 如下代码又问题，linux下出错
 ...
 //    //CMD5 md5;
	// MD5_CTX context;
	// MD5Init (&context);
 //    MD5Update (&context, src, inlen);
 //    MD5Final (&context);
	// memcpy(szDigest,context.state,16);
	// char *p=pszHex;
	// for(int i=0;i<16;i++)
	// {
	// 	sprintf(p,"%02x",szDigest[i]);
	// 	p+=2;
	// }
	// *p=0;
	return pszHex;
...
unsigned char * CMD5::MD5Int(unsigned char *src, unsigned int inlen, unsigned char *pszHex)
{
	MD5_CTX ctx;
	unsigned char md[16];
	// char buf[33]={'\0'};
	...
	MD5Init(&ctx);
	MD5Update(&ctx,(unsigned char *)src,inlen);
	MD5Final(md,&ctx);
...
unsigned char * CMD5::MD5Int2Str(unsigned char *pint, unsigned char *pstr)
{
	int i;
	char *p = (char*)pstr;
	for( i=0; i<16; i++ ){
		sprintf(p,"%02x",pint[i]);
		// printf("%s\n",p);
		p += 2;
	}
	return pstr;
